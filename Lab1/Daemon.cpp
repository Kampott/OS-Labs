...
Daemon* Daemon::getInstance() {
    if (!instance) {
        instance = std::unique_ptr<Daemon>(new Daemon());
    }
    return instance.get();
}

Daemon::Daemon() : interval(30) {}

void Daemon::start(const std::string& config) {
...
void Daemon::moveFiles(const std::string& srcDir, const std::string& destDir, bool olderThan10Min) const {
...
void Daemon::performAction() const {
